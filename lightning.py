...
def fmix_loss(input, y1, index, lam, train=True, reformulate=False):
    ...
    if train and not reformulate:
        y2 = y1[index]
        return CrossEntropyLossOneHot()(input, y1) * lam + CrossEntropyLossOneHot()(input, y2) * (1 - lam)
    else:
        return CrossEntropyLossOneHot()(input, y1)

    # if train and not reformulate:
    #     y2 = y1[index]
    #     return PolyLoss(softmax=True)(input, y1) * lam + PolyLoss(softmax=True)(input, y2) * (1 - lam)
    # else:
    #     return PolyLoss(softmax=True)(input, y1)
...
class CrossEntropyLossOneHot(nn.Module):
    ...
    def forward(self, preds, labels):
        return torch.mean(torch.sum(-labels * self.log_softmax(preds), -1))     # NLLLoss() => https://blog.csdn.net/qq_22210253/article/details/85229988
